...
Volume data tekstual yang dihasilkan pengguna meningkat sangat pesat dengan berbagai format seperti catatan kuliah, log aplikasi, dan dokumentasi proyek. Namun, pencarian teks secara manual pada banyak berkas PDF masih dilakukan dengan cara tradisional, membuka satu per satu file dan menggunakan fitur find bawaan editor—yang memakan waktu, rawan kesalahan, dan tidak efisien. Proyek ini mengusulkan TextSearch, sebuah aplikasi web service pencarian teks multi-berkas yang memungkinkan pengguna melakukan pencarian unlimited kata kunci secara bersamaan dengan hasil yang kontekstual dan ter-highlight.

Aplikasi ini dibangun menggunakan Rust sebagai bahasa pemrograman backend, dipilih karena kombinasi kinerja tinggi, keamanan memori, dan dukungan excellent terhadap pemrograman concurrent dan parallel. Framework web Rocket digunakan untuk membangun API, sementara library Rayon memungkinkan pencarian per-keyword berjalan secara paralel di multiple CPU cores. Pada aspek frontend, Vue.js digunakan untuk memberikan pengalaman interaktif yang responsif. Integrasi konsep pemrograman fungsional diterapkan konsisten melalui penggunaan iterator chains, higher-order functions, immutable data, dan pure functions, sehingga menghasilkan kode yang deklaratif, mudah diuji, dan maintainable.

Keunikan solusi terletak pada tiga aspek utama: (1) No-Upload Paradigm—dokumen PDF pre-loaded dari server saat startup tanpa perlu upload dari pengguna, memungkinkan pencarian instant; (2) Functional Architecture—backend dibangun dengan pure functions dan immutable data yang memudahkan parallel processing; dan (3) Multi-Level Parallelism—pencarian per-keyword berjalan paralel berbeda dengan sistem tradisional yang hanya paralel di level dokumen. Hasil implementasi menunjukkan bahwa TextSearch berhasil memberikan pengalaman pencarian yang cepat, aman, dan user-friendly, dengan benchmark performa yang membandingkan eksekusi parallel vs sequential search. Secara keseluruhan, proyek ini membuktikan bahwa pendekatan fungsional dalam ekosistem Rust dapat menghasilkan aplikasi yang tidak hanya excellent dalam performa tetapi juga maintainable, scalable, dan praktis digunakan.
